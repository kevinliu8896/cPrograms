        ...
        do{
            ...
            for(i = 0; i < 9; i ++){ // for loop
                if(missing_array[i] != 0){ // if statement to find the value in the array that's not 0
                    ...
                    missing_count ++; // increment of missing_count by 1 each iterationg
                } // end if
            } // end for
            printf("\n"); // print new line
            ...
            //       (COMPLETE THIS PART)
            if(game_board[(r - 1) * 3 + (c - 1)] == 63){ // if statement
                temp_board[(r - 1) * 3 + (c - 1)] = v; // update the value of the 1d temp board list

                for(i = 0; i < 9; i ++){ // for loop to update the game_board from the temp_board
                    game_board[i] = temp_board[i]; // set game_board[i] equal to temp_board[i]
                    if(missing_array[i] == v){ // if statement when the value used is in the missing array
                        missing_array[i] = 0; // set missing_array[i] t 0
                    } // end if statement
                } // end for loop
                continue; // continue
            } // end if
